...
# 읽기 전용이므로 테이블의 삽입, 수정, 삭제 작업의 제약을 걸 때 사용가능하다.
# CREAT VIEW 뷰이름 AS
# SELECT 필드이름~~ FROM 테이블  WHERE 조건
...
# INDEX
# 테이블의 검색속도를 향상 시켜주는 기능을 담당하는 요소
# 인덱스가 적용되어있는 필드의 경우 해당 필드를 조건으로 검색을 시도 할 때 
# 검색속도가 향상됨 

# 인덱스가 적용되어있는 필드를 수정할 경우 인덱스도 함께 변경되어
# 검색 속도에 영향을 미칠 수 있음
# CREATE INDEX 인덱스명
# ON 테이블명 (필드명, ...);
...
ON namgu (세대수); 
# 인덱스를 걸어 놨으면 오롯이 세대수를 가지고 검색 할 때만...
...
# GRANT
#사용자에게 권한을 부여하는 쿼리문 
# GRANT 권한1, ... ON 데이터 베이스.테이블 TO 사용자이름@호스트;
...
#REVOKE
# 사용자로부터 권한을 회수하는 쿼리문 
# REVOKE 권한 1, ... ON 데이터베이스.테이블 FROM 사용자이름@호스트;
...
# ROLLBACK
# 현재 트랜잭션을 취소하는 쿼리문 
# AUTOCOMMIT 풀려면 Edit 에서 SQL EXcustion에 new connection  use auto commit mode 체크를 풀면됨 
